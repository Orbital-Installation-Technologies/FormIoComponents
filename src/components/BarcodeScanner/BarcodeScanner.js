...
export default class BarcodeScanner extends FieldComponent {
  ...
  render() {
    ...
    // Load Material Icons font immediately
  ...
  validateAndSetDirty() {
    ...
    if (!valid) {
      ...
      console.log("[BarcodeScanner][L170] setCustomValidity([], true) invoked to clear errors");
  ...
  setCustomValidity(errors, dirty = false) {
    ...
    if (isClearing && !allowClear) {
      console.log("[BarcodeScanner][setCustomValidity blocked]", {
        errors,
        dirty,
        manualErrors: this._manualErrors,
        stack: new Error().stack,
      });
    ...
    console.log("[BarcodeScanner][setCustomValidity apply]", { errors, dirty, allowClear });
