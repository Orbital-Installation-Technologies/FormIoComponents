...
export default class ReviewButton extends FieldComponent {
  ...
  validateDataGridRows(dataGrid) {
    ...
    // Battery optimization: Batch redraw operations
    const redrawTasks = [];
    ...
    dataGrid.rows.forEach((row, rowIndex) => {
      ...
      // Battery optimization: Combine both everyComponent loops into one
      if (panelComponent.everyComponent) {
        panelComponent.everyComponent((comp) => {
          ...
          // Battery optimization: Clear validity first, then validate
    ...
    // Battery optimization: Batch redraw operations using requestAnimationFrame
    if (redrawTasks.length > 0) {
      requestAnimationFrame(() => {
        redrawTasks.forEach(task => task());
      });
    }
  }
  ...
  checkFileComponentHasValue(comp) {
    ...
    // Battery optimization: Check data sources first (faster than DOM queries)
    ...
    // Battery optimization: Early return if value found in data (avoid DOM queries)
    if (hasValue) return true;
    
    // Battery optimization: Only query DOM if element exists and no data value found
    if (!comp.element) return false;
    
    // Battery optimization: Cache regex patterns to avoid recompilation
    const filenamePattern = /\.(jpg|jpeg|png|gif|pdf|doc|docx|txt|zip|rar|mp4|avi|mov|webp|svg|bmp|tiff)$/i;
    const fileSizePattern = /\d+\.?\d*\s*(kb|mb|gb|bytes?)/i;
    
    // Battery optimization: Batch DOM queries - check file inputs first (most common case)
    ...
    // Battery optimization: Use single query with combined selectors where possible
    const uploadedFilesElements = comp.element.querySelectorAll('.file-row, .uploaded-file, [data-file-id]');
    if (uploadedFilesElements.length > 0) {
      return true;
    }
    
    // Battery optimization: Early exit for text content check (cheaper than multiple queries)
    const allText = comp.element.textContent || '';
    if (allText.trim().length > 0) {
      // Battery optimization: Check text content patterns before expensive DOM queries
      const hasFileContent = !allText.includes('No file') && 
      ...
      if (hasFileContent && filenamePattern.test(allText)) {
        return true;
      }
    }
    
    // Battery optimization: Only do expensive queries if needed
    if (isMultiple) {
      // Battery optimization: Use more specific selector to reduce query scope
      const tableRows = comp.element.querySelectorAll('table tbody tr');
      if (tableRows.length > 0) {
        for (let i = 0; i < Math.min(tableRows.length, 10); i++) { // Limit to first 10 rows
          const rowText = tableRows[i].textContent || '';
          if (rowText && (filenamePattern.test(rowText) || fileSizePattern.test(rowText)) &&
              !rowText.includes('File Name') && !rowText.includes('Size')) {
            return true;
          }
        }
      }
      
      // Battery optimization: Check for file items with single query
      const fileItems = comp.element.querySelectorAll('.file-item, .uploaded-file-item, .file-preview, [data-file]');
      if (fileItems.length > 0) {
        return true;
      }
    } else {
      // Battery optimization: For single file, check file lists more efficiently
      const fileLists = comp.element.querySelectorAll('.file-list, .uploaded-files');
      for (let i = 0; i < fileLists.length; i++) {
        const listText = fileLists[i].textContent || '';
        if (listText.trim() && 
            !listText.includes('File Name') && 
            !listText.includes('Size') &&
            !listText.includes('Drop files') &&
            !listText.includes('Browse Files')) {
          return true;
        }
      }
    }
    
    // Battery optimization: Last resort - check for file-related elements (most expensive)
    const fileElements = comp.element.querySelectorAll('[data-file], [data-upload]');
  ...
  isComponentVisible(fieldPath) {
    ...
    try {
      ...
      // Battery optimization: Cache component lookup to avoid repeated everyComponent calls
      const cacheKey = `visible_${fieldPath}`;
      if (this._domCache && this._domCache[cacheKey]) {
        component = this._domCache[cacheKey];
        // Verify component still exists
        if (!component || !component.component) {
          delete this._domCache[cacheKey];
          component = null;
        } else {
          // Return cached visibility state
      ...
      if (fieldPath.startsWith('data.')) {
        ...
        // Battery optimization: Use find instead of everyComponent for single lookup
        if (this.root.components) {
          const findInComponents = (comps) => {
            for (let i = 0; i < comps.length; i++) {
              if (comps[i].key === key) return comps[i];
              if (comps[i].components) {
                const found = findInComponents(comps[i].components);
                if (found) return found;
              }
            }
            return null;
          };
          component = findInComponents(this.root.components);
        }
      } else if (fieldPath.includes('.')) {
        const pathParts = fieldPath.split('.');
        let currentComponent = this.root;
        
        // Battery optimization: Early exit on invalid path
        for (let i = 0; i < pathParts.length && currentComponent; i++) {
        ...
        if (this.root.components) {
          const findInComponents = (comps) => {
            for (let i = 0; i < comps.length; i++) {
              if (comps[i].key === fieldPath) return comps[i];
              if (comps[i].components) {
                const found = findInComponents(comps[i].components);
                if (found) return found;
              }
      ...
      // Battery optimization: Cache component for future lookups
      if (this._domCache) {
        this._domCache[cacheKey] = component;
      }
  ...
  clearFileComponentErrors(comp) {
    ...
    // Battery optimization: Batch redraw and triggerChange operations
    if (comp.redraw) {
      requestAnimationFrame(() => {
        comp.redraw();
      });
    ...
    // Battery optimization: Batch triggerChange calls
    requestAnimationFrame(() => {
      ...
      let depth = 0;
      while (parent && depth < 10) { // Battery optimization: Limit depth to prevent infinite loops
  ...
  clearErrorsFromFileComponentsWithFiles() {
    ...
    // Battery optimization: Collect file components first, then process in batch
    ...
    // Battery optimization: Process file components in batch using requestAnimationFrame
  ...
  clearErrorsFromNestedFormFileComponents() {
    ...
    // Battery optimization: Collect nested file components first
    const nestedFileComponents = [];
        ...
        if (component.everyComponent) {
          component.everyComponent((nestedComponent) => {
            ...
            if (nestedType === 'file') {
              ...
              if (isRequired) {
                nestedFileComponents.push(nestedComponent);
              }
            }
          });
        }
    ...
    // Battery optimization: Process nested file components in batch
    if (nestedFileComponents.length > 0) {
      requestAnimationFrame(() => {
        nestedFileComponents.forEach(nestedComponent => {
  ...
  countVisibleErrors (invalidFields, invalidComponents){
    ...
    // Battery optimization: Pre-compute normalized paths for invalidFields to avoid repeated normalization
    const normalizedInvalidFields = new Map();
    invalidFields.forEach(field => {
      if (field && typeof field === 'string') {
        const normalized = normalizePath(field);
        if (normalized) {
          normalizedInvalidFields.set(field, normalized);
    ...
    invalidComponents.forEach(comp => {
      if (comp && comp.path) {
        ...
        // Battery optimization: Use pre-computed normalized paths instead of nested forEach
        const compKey = comp.component.key;
        normalizedInvalidFields.forEach((normalized) => {
  ...
  attach(element) {
    ...
    this.addEventListener(this.refs.button, "click", async () => {
      try {
        ...
        // Battery optimization: Cache trimKey function and path normalization
        const trimKey = (k) => typeof k === 'string' ? k.trimEnd() : k;
        const normalizePathForCache = (path) => {
          if (!path) return '';
          return trimKey(path)
            .replace(/\.data\./g, '.')
            .replace(/^data\./, '')
        ...
        leaves.forEach(leaf => {
          if (leaf.comp) {
            ...
            if (path) {
              ...
              // Battery optimization: Use cached normalization function
        ...
        const isComponentVisibleInTree = (component) => {
          ...
          if (compPath) {
            ...
            // Battery optimization: Use cached normalization function
            const normalizedPath = normalizePathForCache(compPath);
            if (normalizedPath && visibleComponentPaths.has(normalizedPath)) return true;
            
            // Battery optimization: Early exit when checking path endings
            if (compKey) {
        ...
        if (validation && validation.errors) {
          // Battery optimization: Convert to array once instead of using Object.entries
          const errorEntries = Object.keys(validation.errors).map(key => [key, validation.errors[key]]);
          
          errorEntries.forEach(([errorPath, errorInfo]) => {
            if (errorInfo && errorInfo.errors && Array.isArray(errorInfo.errors)) {
              ...
              // Battery optimization: Use cached normalization function
              const normalizedErrorPath = normalizePathForCache(errorPath);
              
              // Battery optimization: Pre-compute leaf paths to avoid repeated normalization
              let matchingLeaf = null;
              for (let i = 0; i < leaves.length; i++) {
                const leaf = leaves[i];
                if (!leaf.comp || isContainerComponent(leaf.comp, leaf.path)) {
                  continue;
                }
                
                const leafPath = normalizePathForCache(leaf.path || '');
                const leafKey = trimKey(leaf.comp?.key || leaf.comp?.component?.key || '');
                
                // Exact match
                if (leafPath === normalizedErrorPath || leafKey === normalizedErrorPath) {
                  matchingLeaf = leaf;
                  break;
                }
        ...
        // Battery optimization: Use passive listener and store reference for cleanup
        ...
        // Battery optimization: Store handler reference for potential cleanup
        if (!this._eventHandlers) {
          this._eventHandlers = [];
        }
        this._eventHandlers.push({ element: document, event: 'mousedown', handler: mousedownHandler });
  ...
  destroy() {
    ...
    // Battery optimization: Remove event listeners to prevent memory leaks
    if (this._eventHandlers) {
