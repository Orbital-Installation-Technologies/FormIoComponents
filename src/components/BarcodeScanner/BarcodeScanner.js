...
export default class BarcodeScanner extends FieldComponent {
  ...
  constructor(component, options, data) {
    ...
    this.errorMessage = "";
    this._lastBoxes = null;      // array of smoothed polygons from Quagga
    this._lastCode = null;       // decoded string
    this._videoDims = null;      // { width, height } of the video
    this._smoothedBoxes = null;  // intermediate smoothing storage
    this._lostCounts = null;     // track lost frames per box
    // Smoothing parameters:
    this._SMOOTH_ALPHA = 0.1;      // lower alpha = smoother, less responsive
    this._MAX_LOST_FRAMES = 3;    // number of frames to keep a polygon after detection lost
  ...
  _matchAndSmoothBoxes(rawBoxes) {
    ...
    oldCentroids.forEach((oldC, i) => {
      ...
      const MAX_CENTROID_DIST_SQ = 250 * 250; // tighter threshold
